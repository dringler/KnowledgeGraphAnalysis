...
def countInstances(s,p,o):
	...
	if(p==rdfType):
		if (not s in sInstances):
			sInstances.add(s)
		if (o in sClasses):
			if (not s in sInstancesOfClasses):
				sInstancesOfClasses.add(s)
...
try:
	...
	f.close()
	print('DONE')
	print('#triples: {}, #nodes: {}, #namespaceNodes: {}, #properties: {}, #classes: {}, #instances: {}, #instancesOfClasses: {}, avgIndegree: {}, medianIndegree: {}, avgOutdegree: {}, medianOutdegree: {}'.format(sTriples, len(sNodes), len(sNodesNamespace), len(sProperties), len(sClasses), len(sInstances), len(sInstancesOfClasses), getAvg(indegreeDict), getMedian(indegreeDict), getAvg(outdegreeDict), getMedian(outdegreeDict)))
	#print sNodes
	#print sProperties
	#print sClasses
except:
